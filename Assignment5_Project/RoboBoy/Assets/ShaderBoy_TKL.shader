Shader "Unlit/TotallyOriginalShader"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
			...
			{
				float2x2 e;
				return e = float2x2(_scale.x,0.0,0.0,_scale.y); 
			}
			...
			*/
			...
			fixed4 frag (v2f i) : SV_Target
            {
				...
				float e = ridgedMF(float2(0.5,0.5)*(ridgedMF(float2(uv.x*0.8,uv.y*0.5-time*0.1)))+n*ridgedMF(float2(uv.x*1.,uv.y+time*0.002)*ridgedMF(float2(uv.x*0.5,uv.y*0.5+time*0.02))));
    
    
				//VERSION 2 :
				//float2 uv2 = float2(uv.x*0.5,uv.y*0.5-time*0.002);
			/*	e = ridgedMF(float2(uv.x*0.5*uv.y,uv.y*0.5)
						 *(ridgedMF(float2(uv.x*2.2,uv.y*2.0+time*0.01)))
						 +n
						 *ridgedMF(float2(uv)*ridgedMF(float2(uv2))));
				
				//simplify version : */
				//e = ridgedMF(ridgedMF(uv2));
    
    
				static float3 col1 = float3(0.1,0.1,0.4);
				float3 col2 = float3(1.9,0.8,1.-r/2);
